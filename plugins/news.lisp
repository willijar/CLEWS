...
;;;;
;;;; $Id: news.lisp,v 1.1 2005/03/10 20:13:09 willijar Exp $

;Allow adding of global messages with a time limit - controlled by acl
;- Could have a news application tied to it and have the plugin display links
;to it.

(in-package :clews)

(defclass news-plugin (plugin)
  ((messages
    :type list
    :initform nil
    :documentation
    "List of messages - which are lists of a username, some text and a
datetime"))
  (:default-initargs :id :chatterbox-plugin)
  (:documentation "Instant messaging type plugin"))

(defmethod chatterbox-messages(self)
  "returns active list of messages in a chatterbox -  also removes old ones"
  (let* ((start (- (get-universal-time)
                   (property self :message-duration 86400) ))
         (max (1- (property self :max-no-messages 100)))
    ...
    (let ((my-messages '()))
      ...
      `((p ,@(when my-messages
        ...
        ((form :method "POST")
         ...
         ((input :name ,(id self) :value "Send" :width 80 :type "submit")))))))
